            ...
            buttons.get(i).setOnAction(new EventHandler<ActionEvent>() {
                ...
                public void handle(ActionEvent event) {
                    for(int j=0; j<nume.size(); j++){
                        ...
                        TempOrderService.addOrder(UserService.returnNume(RAMService.returnId(nume.get(nr).getText())),MainPage.getUsernameFromMain(), nume.get(nr).getText(),RAMService.returnInteresati(nume.get(nr).getText()),UserService.returnId(MainPage.getUsernameFromMain()));
                        return;
                    }
                }
            });
