            ...
            buttons.get(i).setOnAction(new EventHandler<ActionEvent>() {
                ...
                public void handle(ActionEvent event) {
                    for(int j=0; j<nume.size(); j++){
                        ...
                        TempOrderService.addOrder(UserService.returnNume(ProcessorsService.returnId(nume.get(nr).getText())),MainPage.getUsernameFromMain(), nume.get(nr).getText(),ProcessorsService.returnInteresati(nume.get(nr).getText()),UserService.returnId(MainPage.getUsernameFromMain()));
                        return;
                    }
                }
            });
